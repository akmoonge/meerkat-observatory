...
[2] 모순 지수 (Contradiction Index) 병렬 출력 — 박스와 무관, 라이브 매매 시그널
    ...
    - R (펀더멘털 견고): 시그모이드 0~1. 야드커브/신용/고용/LEI-CEI 결합
...
# V6.6.6: BD 야드커브 조건 (현재 anchor "늦여름" 출력 위해 recovering 도 인정 시도)
...
def compute_contradiction_index(sig):
    ...
    R: 시그모이드 0~1 (야드커브, 신용, 고용, LEI/CEI)
...
if __name__ == "__main__":
    ...
    print("  R = 0.3×야드커브 + 0.3×신용 + 0.2×고용 + 0.2×CFNAI")
