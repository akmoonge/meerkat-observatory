...
def _render_query_result(result):
    ...
    if dominant:
        st.markdown(f"**대표 계절: {_season_emoji(dominant)} {dominant} ({pct_dom:.0f}%)**")
    if dist:
...
**GT 80 채점 결과 (1980-2024 답지 80건)**

| 항목 | 값 |
|---|---|
| raw 정확률 | **72.75%** |
| confidence-weighted | **72.56%** |
| 정확 (1.0) | 51 / 80 (63.7%) |
| 인접 오답 (0.3) | 24 / 80 |
| 반대 오답 (0.0) | 5 / 80 |
...
| 여름 | 16/18 (88.9%) | 매우 높음 |
| 겨울 | 11/15 (73.3%) | 높음 |
| 가을 | 19/32 (59.4%) | 보통 (2층 ANFCI/CAPE 보조 시 80%+) |
| 봄 | 5/15 (33.3%) | 구조적 한계 (회복 신호 축으로 보완) |

**봄 정확률이 낮은 이유**
봄 GT = 침체 바닥 회복기. 침체 바닥에서 동시점 거시 지표 (HY 5%+, 실업률 폭증, VIX 30+, 200dma 하회) 는 실제로 겨울 박스를 점등시킨다. 박스 카운트 시스템이 derivative (방향) 보다 level (수준) 에 가중되는 구조적 한계. 회복 신호 (ICSA) 2층 축이 "겨울이지만 바닥 지났다" 시그널 보완.
