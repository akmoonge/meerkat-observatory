...
EASY={"2Y-10Y 스프레드":"2Y-10Y 스프레드 — 2년-10년 금리차. 마이너스면 형이 걱정하는 거다.",
      "3M-10Y 스프레드":"3M-10Y 스프레드 — 3개월-10년. 이게 역전되고 침체 안 온 적 없다.",
      "3M-2Y 스프레드":"3M-2Y 스프레드 — 3개월-2년. 연준이 너무 올렸는지 보는 지표.",
      ...
      "2Y-10Y ΔΔ":"2Y-10Y ΔΔ — 장단기금리차의 변화 속도가 빨라지는지 느려지는지.",
...
_TIP_CL_BOND = ("<b>채권/금리</b>\n2Y-10Y 스프레드 + 3M-10Y 스프레드 + FF금리 6M 변화 + 실질금리. "
"채권이 형이다. 형 말을 먼저 들어라. "
"3M-10Y 역전은 거시가 가장 신뢰하는 침체 예측 지표다 — 이게 역전되고 침체 안 온 적이 없다. "
...
_CARD_RANGES = {
    "2Y-10Y 스프레드":  "<b>2Y-10Y 스프레드</b>\n🔴 역전 심화   <-40bp\n🔴 역전        -40~0\n🟡 해소 중     0~30\n🟢 정상        ≥30",
    "3M-10Y 스프레드":  "<b>3M-10Y 스프레드</b>\n🔴 역전 심화   <-40bp\n🔴 역전        -40~0\n🟡 해소 중     0~30\n🟢 정상        ≥30",
    "3M-2Y 스프레드":   "<b>3M-2Y 스프레드</b>\n🔴 역전        >0bp\n🟢 역전 직전   -50~0\n🟢 정상        ≤-50",
    "DXY":             "<b>DXY (달러 인덱스)</b>\n🟢 약세        <95\n🟡 중립        95~105\n🔴 강세        105~110\n🔴 극단 강세   ≥110",
    "원/달러":          "<b>원/달러 환율</b>\n🟢 강세        <1100\n🟢 정상        1100~1300\n🟡 약세        1300~1450\n🔴 극단 약세   ≥1450",
    "연방기금금리":     "<b>연방기금금리</b>\n🟢 완화        <1%\n🟡 중립        1~3\n🟡 제한적      3~5\n🔴 긴축        ≥5",
...
# ─── 금리곡선 6상태 (A=2Y-10Y / B=3M-10Y / C=3M-2Y 역전 부호 조합) ───
...
_CURVE_STATE = {
    ...
    (True, False, False): (
        ...
        ("가장 흔한 초기 경고다. 가을 진입 신호.<br><br>"
         ...
         "22년 4~10월이 이 상태였다. 2Y-10Y가 먼저 역전되고 한참 뒤에 3M-10Y가 따라갔다.<br><br>"
         "역사적 출현: 2006 중반, 2019 초, 2022 4~10월"),
    ),
    (False, False, True): (
        "3M-2Y 단독 역전", "2Y < 3M < 10Y", "gold",
    ...
    (True, True, False): (
        ...
        ("채권시장의 경고등이 빨간불이다.<br><br>"
         ...
         "이게 82년 11월 상황이다. 2Y-10Y 역전 67bp, 40년 만의 최대. 근데 그게 인플레 고점 근처였고 주가 저점 근처였다.<br><br>"
...
def _curve_state(t10y2y_v, t10y3m_v, t3m2y_v):
    """(2Y-10Y FRED, 3M-10Y FRED, 3M-2Y 코드 계산) 부호 → _CURVE_STATE 매칭."""
...
# ═══ TRENDS ═══
RIB = {"2Y-10Y": False, "3M-10Y": False, "3M-2Y": True, "DXY": True, "KRW": True, "VIX": True, "HY": True,
...
def mac_sc(d):
    ...
    # V3.6 신규: 3M-10Y 스프레드 (거시가 가장 신뢰하는 침체 예측). t와 동일 패턴 (역전 심화 = 고득점).
    ...
    # V3.6 가중치: 채권/금리 클러스터 내 t+tm 분리 (2Y-10Y 기본, 3M-10Y 침체 예측 핵심)
    W = {
        "t":   7,   # F1 2Y-10Y 스프레드 — 장단기역전 기본
        "tm":  5,   # F1 3M-10Y 스프레드 — "거시가 가장 신뢰하는 침체 예측" (t에서 3 이관)
        "f":  10,   # F6 FF금리 6M 변화
...
# 5클러스터 서브스코어
_CLUSTERS = {
    "채권/금리": ["t", "tm", "f", "r"],   # V3.6: tm (3M-10Y) 추가
    "밸류에이션": ["p", "c", "bf"],
    "스트레스": ["h", "v", "g"],
    "실물": ["u", "gd", "cf"],
    "반도체": ["sd", "sr"],
...
def _build_state_at_offset_full(raw, offset):
    ...
    # yield_curve (3M-10Y 기준, date-based)
...
def main():
    ...
    gd = {"t10y2y": t10y2y, "t10y3m": t10y3m,  # V3.6: 3M-10Y 신규
    ...
    def _export_html(data, title="미어캣의 관측소", charts=None, section=None):
        ...
        if data.get("주요지표_방향성"):
            _dnames = {"VIX": "VIX", "HY_bp": "HY (bp)", "2Y10Y_bp": "2Y-10Y (bp)", "DXY": "DXY (%)", "SOX_SPX": "SOX/SPX (%)", "KRW": "원/달러 (%)"}
...
<hr><p style="color:#666;font-size:12px">미어캣의 관측소 V{VERSION} · 자동 생성</p></body></html>"""
    ...
    with tabs[0]:
        ...
        with _mx_c2:
            ...
            _DELTA_NAMES = {"VIX": "VIX", "HY_bp": "HY", "2Y10Y_bp": "2Y-10Y", "DXY": "DXY", "SOX_SPX": "SOX/SPX", "KRW": "원/달러"}
            def _dcol(k, v):
        ...
        with kr[2]:
            ...
            _ic("2Y-10Y 스프레드", _v, s, c, d, _raw_dd_trend("T10Y2Y", tbar(ctrends(fd.get("T10Y2Y"), mode="abs_bp"), "2Y-10Y")))
        with kr[3]:
        ...
        if deep:
            ...
            with d1[0]:
                ...
                _ic("3M-10Y 스프레드", _v, s2, c2, d2, tbar(ctrends(fd.get("T10Y3M"), mode="abs_bp"), "3M-10Y"))
            with d1[1]:
                td = ""
                if t3m2y is not None:
...
KRW: FRED DEXKOUS | WTI: FRED DCOILWTICO | DXY: yfinance DX-Y.NYB</div>""", unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        try:
            _t10y2y_now = float(sd_["2Y-10Y"].iloc[-1]) if "2Y-10Y" in sd_ else None
            _t10y3m_now = float(sd_["3M-10Y"].iloc[-1]) if "3M-10Y" in sd_ else None
            _t3m2y_now = float(sd_["3M-2Y"].iloc[-1]) if "3M-2Y" in sd_ else None
        except Exception:
